...
module Reinitializer
    ...
    def set_board_position(game_data)
        game.game_state.board.positions = game_data[:board_positions]
    end

    def set_current_player(game_data)
        game.game_state.current_player = game_data[:current_player_token] == :X ? game.game_state.player_1 : game.game_state.player_2
    end

    def set_lite_3_data(game_data)
        game.game_state.game_mode.player_1_moves = game_data[:player_1_moves] 
        game.game_state.game_mode.player_2_moves = game_data[:player_2_moves] 
    end
end 