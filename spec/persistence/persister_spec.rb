...
describe 'Persistence' do
    ...
    context 'resume' do
        ...
        it 'knows the correct current player ' do
            expect(restarted_game.current_player.token).to eq :O
        end

        it 'knows the game mode' do
            expect(restarted_game.game_mode.is_a?(Lite3)).to eq true
        end

        it 'knows player_1 moves' do
            expect(restarted_game.game_mode.player_1_moves).to eq MockTestHelper.player_1_moves(game)
        end

        it 'knows player_2 moves' do
            expect(restarted_game.game_mode.player_2_moves).to eq MockTestHelper.player_2_moves(game)
        end
        ...
        it 'knows board positions' do
            expect(restarted_game.board.positions).to eq MockTestHelper.positions(game)
        end
    end
end